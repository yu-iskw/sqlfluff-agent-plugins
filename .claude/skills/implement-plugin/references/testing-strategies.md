...
```bash
.claude/skills/implement-plugin/scripts/validate-manifest.sh
.claude/skills/implement-plugin/scripts/check-structure.sh
```

## 2) Runtime Load Check with `--plugin-dir`

```bash
.claude/skills/implement-plugin/scripts/test-plugin-load.sh .
```
